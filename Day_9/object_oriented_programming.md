...
By default, Python objects store attributes in a dictionary (`__dict__`), which consumes extra memory. If you create many instances, use `__slots__` to declare fixed attributes and eliminate `__dict__`.
...
- **Advantage**: Less memory usage, slightly faster attribute access.
- **Disadvantage**: Cannot add new attributes dynamically, care needed with inheritance.
